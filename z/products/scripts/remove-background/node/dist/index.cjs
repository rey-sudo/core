...
async function imageEncode(imageData, quality = 0.8, type = "image/png") {
  const [height, width, channels] = imageData.shape;
  if (channels !== 4)
    throw new Error("Only 4-channel images are supported");
  const image = (0, import_sharp.default)(imageData.data, { raw: { height, width, channels } });
...
var package_default = {
  ...
  scripts: {
    ...
    test: "true",
    resources: "node ./package-resources.mjs",
...
var ConfigSchema = z.object({
  publicPath: z.string().optional().describe("The public path to the wasm files and the onnx model.").default(`file://${import_node_path.default.resolve(`node_modules/${package_default.name}/dist/`)}/`).refine((val) => isURI(val), {
...
async function loadFromURI(uri, config = { headers: { "Content-Type": "application/octet-stream" } }) {
  switch (uri.protocol) {
    ...
    case "https:":
      return await fetch(uri);
    case "file:": {
      const buffer = await (0, import_promises.readFile)(uri);
...
async function loadAsBlob(key, config) {
  ...
  const allChunks = await Promise.all(
    paths.map(async (path2) => {
      const url = ensureAbsoluteURI(path2, config.publicPath);
