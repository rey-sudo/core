...
var _HotWallet = class _HotWallet {
   ...
   */
  constructor(address, rewardAddress, signingKey, publicKey, provider) {
  ...
  static async generateAccountAddressFromMasterkey(masterkey, networkId = import_core2.NetworkId.Testnet, addressType = import_core2.AddressType.BasePaymentKeyStakeKey) {
    if (addressType !== import_core2.AddressType.BasePaymentKeyStakeKey && addressType !== import_core2.AddressType.EnterpriseKey) {
      throw new Error(
    ...
    const address = new import_core2.Address({
      type: addressType,
      networkId,
      paymentPart: {
        type: import_core2.CredentialType.KeyHash,
        hash: import_core2.Hash28ByteBase16.fromEd25519KeyHashHex(
          (await (await paymentKey.toPublic()).toRawKey().hash()).hex()
        )
      },
      delegationPart: addressType === import_core2.AddressType.EnterpriseKey ? void 0 : {
        type: import_core2.CredentialType.KeyHash,
        // 1852H/1815H/0H/2/0
        hash: import_core2.Hash28ByteBase16.fromEd25519KeyHashHex(
  ...
  static async fromMasterkey(masterkey, provider, networkId = import_core2.NetworkId.Testnet, addressType = import_core2.AddressType.BasePaymentKeyStakeKey) {
    const rootKey = import_core2.Bip32PrivateKey.fromHex(masterkey);
    const { address, paymentKey, publicKey } = await this.generateAccountAddressFromMasterkey(
      rootKey,
      networkId,
      addressType
    );
  ...
  async signTransaction(tx, partialSign = true) {
    ...
    const signature = await this.signingKey.toRawKey().sign((0, import_core2.HexBlob)(tx.getId()));
    const tws = new import_core2.TransactionWitnessSet();
    const vkw = new import_core2.VkeyWitness(
      this.publicKey.toRawKey().hex(),
      signature.hex()
    );
    tws.setVkeys(import_core2.CborSet.fromCore([vkw.toCore()], import_core2.VkeyWitness.fromCore));
    return tws;
  }
  /**
   * Requests signed data from the wallet.
...
var HotSingleWallet = class {
  ...
  constructor(paymentSigningKey, networkId, provider, stakeSigningKey) {
    ...
    this.paymentPublicKey = import_core3.Ed25519PublicKey.fromHex(
      (0, import_core3.derivePublicKey)(this.paymentSigningKey)
    );
    if (stakeSigningKey) {
      this.stakeSigningKey = stakeSigningKey;
      this.stakePublicKey = import_core3.Ed25519PublicKey.fromHex(
        (0, import_core3.derivePublicKey)(this.stakeSigningKey)
      );
    }
    this.address = this.address = new import_core3.Address({
      type: import_core3.AddressType.EnterpriseKey,
      networkId: this.networkId,
      paymentPart: {
        type: import_core3.CredentialType.KeyHash,
        hash: (0, import_core3.blake2b_224)((0, import_core3.HexBlob)(this.paymentPublicKey.hex()))
      },
      delegationPart: this.stakePublicKey ? {
        type: import_core3.CredentialType.KeyHash,
        hash: (0, import_core3.blake2b_224)((0, import_core3.HexBlob)(this.stakePublicKey.hex()))
      } : void 0
    });
    this.provider = provider;
  }
  ...
  async signTransaction(tx, partialSign = true) {
    ...
    const signature = (0, import_core3.signMessage)((0, import_core3.HexBlob)(tx.getId()), this.paymentSigningKey);
    const tws = new import_core3.TransactionWitnessSet();
    const vkw = new import_core3.VkeyWitness(
      this.paymentPublicKey.hex(),
      (0, import_core3.Ed25519SignatureHex)(signature)
    );
