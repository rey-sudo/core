...
var TxBuilder = class {
  ...
  constructor(params) {
    ...
    this.redeemers = import_core3.Redeemers.fromCore([]);
    ...
    this.params = params;
    this.body = new import_core3.TransactionBody(
      import_core3.CborSet.fromCore([], import_core3.TransactionInput.fromCore),
      [],
      0n,
      void 0
    );
  ...
  addReferenceInput(utxo) {
    const referenceInputs = this.body.referenceInputs() ?? import_core3.CborSet.fromCore([], import_core3.TransactionInput.fromCore);
  ...
  addInput(utxo, redeemer, unhashDatum) {
    ...
    for (const redeemer2 of redeemers) {
      if (redeemer2.tag() == import_core3.RedeemerTag.Spend && redeemer2.index() >= BigInt(insertIdx)) {
    ...
    if (redeemer !== void 0) {
      if (key.type == import_core3.CredentialType.KeyHash) {
        throw new Error(
      ...
      redeemers.push(
        import_core3.Redeemer.fromCore({
          index: insertIdx,
          purpose: import_core3.RedeemerPurpose["spend"],
      ...
      if (key.type == import_core3.CredentialType.ScriptHash) {
        this.requiredNativeScripts.add(key.hash);
      } else {
        this.requiredWitnesses.add((0, import_core3.HashAsPubKeyHex)(key.hash));
      }
    }
    const scriptRef = utxo.output().scriptRef();
    if (scriptRef) {
  ...
  addMint(policy, assets2, redeemer) {
    const insertIdx = this.insertSorted(
      this.consumedMintHashes,
      (0, import_core3.PolicyIdToHash)(policy)
    );
    const mint = this.body.mint() ?? /* @__PURE__ */ new Map();
    for (const asset of mint.keys()) {
      if (import_core3.AssetId.getPolicyId(asset) == policy) {
        throw new Error("addMint: Duplicate policy!");
      }
    }
    for (const [key, amount] of assets2.entries()) {
      mint.set(import_core3.AssetId.fromParts(policy, key), amount);
    }
    this.body.setMint(mint);
    if (redeemer) {
      this.requiredPlutusScripts.add((0, import_core3.PolicyIdToHash)(policy));
      const redeemers = [...this.redeemers.values()];
      for (const redeemer2 of redeemers) {
        if (redeemer2.tag() == import_core3.RedeemerTag.Mint && redeemer2.index() >= insertIdx) {
          redeemer2.setIndex(redeemer2.index() + 1n);
        }
      }
      redeemers.push(
        import_core3.Redeemer.fromCore({
          index: insertIdx,
          purpose: import_core3.RedeemerPurpose["mint"],
      ...
      this.requiredNativeScripts.add((0, import_core3.PolicyIdToHash)(policy));
    }
    return this;
  }
  /**
  ...
  checkAndAlterOutput(output) {
    {
      let byteLength2 = BigInt(output.toCbor().length / 2);
      let minAda = BigInt(this.params.coinsPerUtxoByte) * (byteLength2 + 160n);
      ...
      while (coin < minAda) {
        ...
        output = new import_core3.TransactionOutput(output.address(), amount);
        if (datum) {
        ...
        byteLength2 = BigInt(output.toCbor().length / 2);
        minAda = BigInt(this.params.coinsPerUtxoByte) * (byteLength2 + 160n);
   ...
   * @returns {TxBuilder} The same transaction builder
   */
  payLovelace(address, lovelace) {
    assertPaymentsAddress(address);
    const paymentAddress = (0, import_core3.getPaymentAddress)(address);
    this.addOutput(
      import_core3.TransactionOutput.fromCore({
  ...
  lockLovelace(address, lovelace, datum, scriptReference) {
    ...
    this.addOutput(
      import_core3.TransactionOutput.fromCore({
        address: paymentAddress,
        value: { coins: lovelace },
        datum: !("__opaqueString" in datum) ? datum.toCore() : void 0,
        datumHash: "__opaqueString" in datum ? datum : void 0,
  ...
  buildTransactionWitnessSet() {
    const tw = new import_core3.TransactionWitnessSet();
    const scriptLookup = {};
    ...
    for (const requiredScriptHash of this.requiredPlutusScripts) {
      ...
      if (lang == 1) {
        this.usedLanguages[import_core3.PlutusLanguageVersion.V1] = true;
      } else if (lang == 2) {
        this.usedLanguages[import_core3.PlutusLanguageVersion.V2] = true;
      } else if (lang == 3) {
        this.usedLanguages[import_core3.PlutusLanguageVersion.V3] = true;
      } else if (!lang) {
        throw new Error(
    ...
    if (sn.length != 0) {
      const cborSet = import_core3.CborSet.fromCore([], import_core3.NativeScript.fromCore);
      cborSet.setValues(sn);
      tw.setNativeScripts(cborSet);
    }
    if (s1.length != 0) {
      const cborSet = import_core3.CborSet.fromCore([], import_core3.PlutusV1Script.fromCore);
      cborSet.setValues(s1);
      tw.setPlutusV1Scripts(cborSet);
    }
    if (s2.length != 0) {
      const cborSet = import_core3.CborSet.fromCore([], import_core3.PlutusV2Script.fromCore);
      cborSet.setValues(s2);
      tw.setPlutusV2Scripts(cborSet);
    }
    if (s3.length != 0) {
      const cborSet = import_core3.CborSet.fromCore([], import_core3.PlutusV3Script.fromCore);
    ...
    this.requiredWitnesses.forEach((_) => {
      requiredWitnesses.push([
        (0, import_core3.Ed25519PublicKeyHex)("0".repeat(64)),
        (0, import_core3.Ed25519SignatureHex)("0".repeat(128))
      ]);
    });
    ...
    tw.setVkeys(import_core3.CborSet.fromCore(requiredWitnesses, import_core3.VkeyWitness.fromCore));
    tw.setRedeemers(this.redeemers);
    const plutusData = import_core3.CborSet.fromCore([], import_core3.PlutusData.fromCore);
  ...
  getPitch(spareAmount = 0n) {
    ...
    let inputValue = new import_core3.Value(withdrawalAmount);
    let outputValue = new import_core3.Value(BigIntMax(this.fee, this.minimumFee));
    const mintValue = new import_core3.Value(0n, this.body.mint());
    ...
    this.body.outputs()[this.changeOutputIndex] = new import_core3.TransactionOutput(
      this.changeAddress,
      zero()
    );
    ...
    for (const cert of this.body.certs()?.values() || []) {
      switch (cert.kind()) {
        case 0:
          outputValue = merge(
            outputValue,
            new import_core3.Value(BigInt(this.params.stakeKeyDeposit))
          );
          break;
        case 1:
          inputValue = merge(
        ...
        case 3:
          if (this.params.poolDeposit) {
            outputValue = merge(
              outputValue,
              new import_core3.Value(BigInt(this.params.poolDeposit))
            );
          }
          break;
        case 4:
    ...
    if (spareAmount != 0n) {
      return merge(tilt, new import_core3.Value(-spareAmount));
    }
    return tilt;
  }
  /**
  ...
  getScriptDataHash(tw) {
    ...
    if (datums.length > 0 || redeemers.length > 0) {
      ...
      const usedCostModels = new import_core3.Costmdls();
      for (let i = 0; i <= Object.keys(this.usedLanguages).length; i++) {
        if (this.usedLanguages[i]) {
          ...
          usedCostModels.insert(new import_core3.CostModel(i, cm));
        }
      }
      writer.writeEncodedValue(
        Buffer.from(usedCostModels.languageViewsEncoding(), "hex")
      );
      return (0, import_core3.blake2b_256)(writer.encodeAsHex());
    }
    return void 0;
  }
  /**
  ...
  balanceChange(excessValue) {
    ...
    const output = new import_core3.TransactionOutput(this.changeAddress, excessValue);
  ...
  calculateFees(draft_tx) {
    this.fee = BigInt(
      Math.ceil(
        this.params.minFeeConstant + (0, import_core3.fromHex)(draft_tx.toCbor()).length * this.params.minFeeCoefficient
      )
    );
  ...
  prepareCollateral() {
    ...
    for (const input of inputs) {
      ...
      if (utxo) {
        if (utxo.output().amount().coin() >= 10n * 10n ** 6n && utxo.output().address().getProps().paymentPart?.type == import_core3.CredentialType.KeyHash) {
    ...
    const tis = import_core3.CborSet.fromCore([], import_core3.TransactionInput.fromCore);
  ...
  balanceCollateralChange() {
    ...
    const totalCollateral = BigInt(
      Math.ceil(
        this.params.collateralPercentage * Number(BigIntMax(this.fee, this.minimumFee))
      )
    );
    ...
    this.body.setCollateralReturn(
      new import_core3.TransactionOutput(
        this.changeAddress,
        merge(collateralValue, new import_core3.Value(-totalCollateral))
      )
    );
  ...
  async complete() {
    ...
    const inputs = [...this.body.inputs().values()];
    let excessValue = this.getPitch(5000000n);
    let spareInputs = [];
    ...
    this.balanceChange(excessValue);
    const draft_tx = new import_core3.Transaction(this.body, tw, auxiliaryData);
    let draft_size = draft_tx.toCbor().length / 2;
    this.calculateFees(draft_tx);
    excessValue = merge(
      excessValue,
      new import_core3.Value(-BigIntMax(this.fee, this.minimumFee))
    );
    this.balanceChange(excessValue);
    if (this.redeemers.size() > 0) {
      ...
      excessValue = merge(excessValue, new import_core3.Value(-evaluationFee));
      tw.setRedeemers(this.redeemers);
    ...
    tw.setVkeys(import_core3.CborSet.fromCore([], import_core3.VkeyWitness.fromCore));
    return new import_core3.Transaction(this.body, tw, this.auxiliaryData);
  }
  /**
  ...
  addDelegation(delegator, poolId, redeemer) {
    const stakeDelegation = {
      __typename: import_core3.CertificateType.StakeDelegation,
      stakeCredential: delegator.toCore(),
      poolId
    };
    const delegationCertificate = import_core3.Certificate.newStakeDelegation(
      import_core3.StakeDelegation.fromCore(stakeDelegation)
    );
    const certs = this.body.certs() ?? import_core3.CborSet.fromCore([], import_core3.Certificate.fromCore);
    ...
    if (delegatorCredential.type == import_core3.CredentialType.ScriptHash) {
      if (redeemer) {
        ...
        redeemers.push(
          import_core3.Redeemer.fromCore({
            index: 256,
            // todo: fix
            purpose: import_core3.RedeemerPurpose["certificate"],
            data: redeemer.toCore(),
            executionUnits: {
              memory: this.params.maxExecutionUnitsPerTransaction.memory,
              steps: this.params.maxExecutionUnitsPerTransaction.steps
      ...
      this.requiredWitnesses.add((0, import_core3.HashAsPubKeyHex)(delegatorCredential.hash));
    }
    return this;
  }
  /**
  ...
  delegate(poolId, redeemer) {
    ...
    this.addDelegation(import_core3.Credential.fromCore(credential), poolId, redeemer);
    return this;
  }
  /**
  ...
  addRegisterStake(credential) {
    const stakeRegistration = new import_core3.StakeRegistration(
      credential.toCore()
    );
    const registrationCertificate = import_core3.Certificate.newStakeRegistration(stakeRegistration);
    const certs = this.body.certs() ?? import_core3.CborSet.fromCore([], import_core3.Certificate.fromCore);
  ...
  addWithdrawal(address, amount, redeemer) {
    const withdrawalHash = import_core3.Address.fromBech32(address).getProps().paymentPart?.hash;
    if (!withdrawalHash) {
      throw new Error(
    ...
    if (redeemer) {
      ...
      const redeemers = [...this.redeemers.values()];
      for (const redeemer2 of redeemers) {
        if (redeemer2.tag() == import_core3.RedeemerTag.Reward && redeemer2.index() >= insertIdx) {
          redeemer2.setIndex(redeemer2.index() + 1n);
        }
      }
      redeemers.push(
        import_core3.Redeemer.fromCore({
          index: insertIdx,
          purpose: import_core3.RedeemerPurpose["withdrawal"],
      ...
      const key = import_core3.Address.fromBech32(address).getProps().paymentPart;
      if (!key) {
        throw new Error(
          "addWithdrawal: The RewardAccount provided does not have an associated stake credential."
        );
  ...
  addRequiredSigner(signer) {
    const signers = this.body.requiredSigners() ?? import_core3.CborSet.fromCore([], import_core3.Hash.fromCore);
    const values = [...signers.values()];
    values.push(import_core3.Hash.fromCore(signer));
  ...
  getAuxiliaryDataHash(auxiliaryData) {
    return auxiliaryData ? (0, import_core3.blake2b_256)(auxiliaryData.toCbor()) : void 0;
  }
  /**
...
function assertPaymentsAddress(address) {
  ...
  if (props.paymentPart.type == import_core3.CredentialType.ScriptHash) {
    throw new Error(
...
// src/data.ts
var listToCbor = import_core4.PlutusList.prototype.toCbor;
import_core4.PlutusList.prototype.toCbor = function() {
  this.toCbor = listToCbor;
  const res = this.toCbor();
  this.toCbor = import_core4.PlutusList.prototype.toCbor;
  if (res == "9fff") {
    return (0, import_core4.HexBlob)("80");
  }
  return res;
};
...
var Data = {
  ...
  Void: () => import_core4.PlutusData.newConstrPlutusData(new import_core4.ConstrPlutusData(0n, new import_core4.PlutusList())),
  castFrom,
  castTo,
  to,
  from
...
function castTo(struct, schema) {
  if (!schema) throw new Error("Could not type cast struct.");
  if (struct instanceof import_core4.PlutusData) {
    return struct;
  }
  if ("anyOf" in schema) {
    const schemaEnum = schema;
    if (schema["anyOf"].length === 1) {
      return castTo(struct, schema["anyOf"][0]);
    }
    if (isBoolean(schemaEnum)) {
      ...
      return import_core4.PlutusData.newConstrPlutusData(
        new import_core4.ConstrPlutusData(
          struct ? BigInt(trueIdx) : BigInt(falseIdx),
          new import_core4.PlutusList()
        )
      );
      ...
      if (struct === null) {
        return import_core4.PlutusData.newConstrPlutusData(
          new import_core4.ConstrPlutusData(BigInt(noneIdx), new import_core4.PlutusList())
        );
      } else {
        ...
        const someList = new import_core4.PlutusList();
        someList.add(castTo(struct, fields[someIdx]));
        return import_core4.PlutusData.newConstrPlutusData(
          new import_core4.ConstrPlutusData(BigInt(someIdx), someList)
        );
      }
    }
    switch (typeof struct) {
      case "string": {
        ...
        return import_core4.PlutusData.newConstrPlutusData(
          new import_core4.ConstrPlutusData(BigInt(enumIndex), new import_core4.PlutusList())
        );
      }
      case "object": {
        ...
        const plutusList = new import_core4.PlutusList();
        fields.forEach((f) => plutusList.add(f));
        return import_core4.PlutusData.newConstrPlutusData(
          new import_core4.ConstrPlutusData(BigInt(enumEntry["index"]), plutusList)
        );
      }
    }
  }
  ...
  switch (schema["dataType"]) {
    case "integer": {
      ...
      return import_core4.PlutusData.newInteger(struct);
    }
    case "bytes": {
      if (typeof struct !== "string") {
        throw new Error("Could not type cast to bytes.");
      }
      return import_core4.PlutusData.newBytes((0, import_core4.fromHex)(struct));
    }
    case "constructor": {
      if (schema["fields"].length == 0 && typeof struct == "string") {
        ...
        return import_core4.PlutusData.newConstrPlutusData(
          new import_core4.ConstrPlutusData(BigInt(schema["index"]), new import_core4.PlutusList())
        );
      }
      if (isVoidConstructor(schema)) {
        ...
        return import_core4.PlutusData.newConstrPlutusData(
          new import_core4.ConstrPlutusData(0n, new import_core4.PlutusList())
        );
      ...
      const fields = new import_core4.PlutusList();
      ...
      return import_core4.PlutusData.newConstrPlutusData(
        new import_core4.ConstrPlutusData(BigInt(schema["index"]), fields)
      );
    }
    case "list": {
      ...
      if (schema["items"] instanceof Array) {
        ...
        return import_core4.PlutusData.newList(fields);
      } else {
        const arrayType = schema["items"];
        const list = new import_core4.PlutusList();
        struct.forEach((item) => list.add(castTo(item, arrayType)));
        return import_core4.PlutusData.newList(list);
      }
    }
    case "map": {
      ...
      const map = new import_core4.PlutusMap();
      ...
      return import_core4.PlutusData.newMap(map);
    }
  }
  throw new Error("Could not type cast struct.");
}
function castFrom(rawData, schema) {
  ...
  if ("anyOf" in schema) {
    ...
    if (!(kind == import_core4.PlutusDataKind.ConstrPlutusData)) {
  ...
  switch (schema["dataType"]) {
    case "integer": {
      if (kind !== import_core4.PlutusDataKind.Integer) {
        throw new Error("Could not type cast to integer.");
      }
      return rawData.asInteger();
    }
    case "bytes": {
      if (kind !== import_core4.PlutusDataKind.Bytes) {
        throw new Error("Could not type cast to integer.");
      }
      return (0, import_core4.toHex)(rawData.asBoundedBytes());
    }
    case "constructor": {
      if (kind !== import_core4.PlutusDataKind.ConstrPlutusData) {
        throw new Error("Could not type cast to constructor.");
    ...
    case "list": {
      if (kind !== import_core4.PlutusDataKind.List) {
    ...
    case "map": {
      if (!(kind == import_core4.PlutusDataKind.Map)) {
        throw new Error("Could not type cast to map.");
      }
