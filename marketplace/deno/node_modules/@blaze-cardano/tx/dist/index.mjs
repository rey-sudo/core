...
var TxBuilder = class {
  ...
  addMint(policy, assets2, redeemer) {
    ...
    if (redeemer) {
      ...
      const redeemers = [...this.redeemers.values()];
      for (const redeemer2 of redeemers) {
        if (redeemer2.tag() == RedeemerTag.Mint && redeemer2.index() >= insertIdx) {
          redeemer2.setIndex(redeemer2.index() + 1n);
        }
      }
      redeemers.push(
  ...
  calculateFees(draft_tx) {
    this.fee = BigInt(
      Math.ceil(
        this.params.minFeeConstant + fromHex(draft_tx.toCbor()).length * this.params.minFeeCoefficient
      )
    );
  ...
  prepareCollateral() {
    ...
    for (const input of inputs) {
      ...
      if (utxo) {
        if (utxo.output().amount().coin() >= 10n * 10n ** 6n && utxo.output().address().getProps().paymentPart?.type == CredentialType.KeyHash) {
  ...
  async complete() {
    ...
    excessValue = merge(
      excessValue,
      new Value2(-BigIntMax(this.fee, this.minimumFee))
    );
    this.balanceChange(excessValue);
  ...
  addWithdrawal(address, amount, redeemer) {
    ...
    if (redeemer) {
      ...
      const redeemers = [...this.redeemers.values()];
      for (const redeemer2 of redeemers) {
        if (redeemer2.tag() == RedeemerTag.Reward && redeemer2.index() >= insertIdx) {
          redeemer2.setIndex(redeemer2.index() + 1n);
        }
      }
      redeemers.push(
  ...
  getAuxiliaryDataHash(auxiliaryData) {
    return auxiliaryData ? blake2b_256(auxiliaryData.toCbor()) : void 0;
  }
  /**
...
// src/data.ts
var listToCbor = PlutusList.prototype.toCbor;
PlutusList.prototype.toCbor = function() {
...
function castTo(struct, schema) {
  ...
  switch (schema["dataType"]) {
    ...
    case "bytes": {
      ...
      return PlutusData2.newBytes(fromHex2(struct));
    }
    case "constructor": {
