        ...
        for (const maestroUTxO of response.data) {
          ...
          fetch(`${this.url}${query2}?with_cbor=true`, {
            ...
            if (json2) {
              ...
              const txOut = TransactionOutput.fromCbor(
                HexBlob(response2.data.txout_cbor)
              );
              utxos.push(new TransactionUnspentOutput(txIn, txOut));
            } else {
              throw new Error(
...
import {
  RedeemerPurpose,
  RedeemerTag as RedeemerTag2
} from "@blaze-cardano/core";
var Provider = class {
};
var purposeToTag = {
  [RedeemerPurpose.spend]: RedeemerTag2.Spend,
  [RedeemerPurpose.mint]: RedeemerTag2.Mint,
  [RedeemerPurpose.certificate]: RedeemerTag2.Cert,
  [RedeemerPurpose.withdrawal]: RedeemerTag2.Reward,
  [RedeemerPurpose.vote]: RedeemerTag2.Voting,
  [RedeemerPurpose.propose]: RedeemerTag2.Proposing
};
...
var _Kupmios = class _Kupmios {
  ...
  async resolveScript(scriptHash) {
    ...
    switch (result.language) {
      case "native":
        return Script2.newNativeScript(NativeScript.fromCbor(result.script));
      case "plutus:v1":
        return Script2.newPlutusV1Script(new PlutusV1Script2(result.script));
      case "plutus:v2":
        return Script2.newPlutusV2Script(new PlutusV2Script2(result.script));
      case "plutus:v3":
        return Script2.newPlutusV3Script(new PlutusV3Script(result.script));
      default:
